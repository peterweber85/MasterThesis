...
The plots in Figure \ref{fig:conv_plots} (on the next page) show the convergence of some of the models trained, for the different datasets and resolutions (and considering one of the cross-validation folds only). Also, two architectures for the dense layer have been tested: $100$ neurons and $200$ neurons. \textcolor{red}{Why are you changing two parameters from one figure to the next? Wouldn't it be more logical to vary only one parameter in order to show a progression? I think it would be important to show e.g. for base resolution 0.3, lowest resolt and highest resolution for 100 neurons. And then show the same for 200 neurons. Here we could double the number of figures since anyways the next page is blank. On the second page you could do the same for base resolution 1m.}
