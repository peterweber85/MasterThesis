...
Our approach is based on developing a deep learning detector to classify human impact on aerial images. In particular, we exploit recent advances of deep Convolutional Neural Networks that were successfully used for object detection and scene classification. We apply transfer learning by integrating a ResNet pre-trained on ImageNet to perform image classification on datasets of few thousand aerial images that we have manually collected and annotated. Using this image classification pipeline we are able to determine the existence of man-made structure with an accuracy of 95\% at the best resolution.

We study the performance of the deep learning detector for resolutions ranging from 0.3m to 16m. Doing so we observe a linear decrease of the classification accuracy eventually being 87\% at the lowest resolution. Furthermore, we estimate the cost associated to capturing and processing satellite images. This includes the entire pipeline from building and launching a satellite to predicting human impact on images. We estimate that monitoring the entire land surface of the earth at 1m resolution amounts for about \$ 15 million dollars. This cost increases by about two orders of magnitude at the best resolution studied here, and decreases by about one order of magnitude at a resolution of 10m per pixel. 

These results could be further improved by training a deep Convolutional Neural Network on a labeled large scale dataset of remote sensing images. Nevertheless, our results suffice for studying the expansion of human kind using satellite imagery and provide valuable information for designing optimal satellite sensors.
