...
Note that the splitting parameters of the cross-validation, model complexity and the training epochs could be further analyzed in order to fit them better to each of the resolutions. 

\

The plots below show the convergence of the models for different resolutions and datasets (and for one of the cross-validation fold). In general, the NN is able to converge and achieve a good accuracy, although for some particular splits of the data it fails to converge. This is probably due to the fact of having a relative small dataset. Hence, these folds are not considered when computing the final averaged results.

\textcolor{red}{COMPLETE PLOTS OF SOME RESOLUTIONS AND DATASETS}

\textcolor{red}{BETTER EXPLAIN RELATING TO THE PLOTS}

In the next chapter we discuss in detail the results obatined from all these experiments.
